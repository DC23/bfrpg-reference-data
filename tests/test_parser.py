...
def test_paragraph_event_shape(walk_events):
    ...
    bad = [
        e for e in walk_events
        if e[0] == 'p' and (len(e) != 3 or not isinstance(e[1], str) or not isinstance(e[2], str))
    ]
...
def test_table_event_shape(walk_events):
    ...
    bad = [
        e for e in walk_events
        if e[0] == 'table' and (len(e) != 3 or not isinstance(e[2], list) or len(e[2]) == 0)
    ]
...
def test_single_entity_table_starts_with_field(walk_events):
    ...
    for i, e in enumerate(events):
        if e[0] == 'p' and e[1] == 'MonsterHeading' and e[2] == 'Antelope (Herd Animals)':
...
def test_multi_entity_table_has_empty_first_cell(walk_events):
    ...
    events = list(walk_events)
    for i, e in enumerate(events):
        if e[0] == 'p' and e[1] == 'MonsterHeading' and e[2] == 'Ant, Giant (and Huge, Large)':
            nxt = events[i + 1]
            assert nxt[0] == 'table'
            row0 = nxt[2][0]
            assert row0[0] == ''
